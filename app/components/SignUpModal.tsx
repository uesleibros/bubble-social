...
export default function SignUpModal({ visible }) {
	...
	const [isUsernameInvalid, setIsUsernameInvalid] = useState<boolean>(true);
		...
		<div className="fixed top-0 flex items-center justify-center bg-black bg-opacity-50 w-full h-screen sm:h-full z-1">
			<div className="bg-white md:shadow-sm md:rounded-lg flex flex-col justify-between w-[35%] h-[80vh] sm:w-full sm:h-full">
				<div className="flex items-center justify-center px-3 py-2 shrink-0 mb-5 border-b">
					...
					<h2 className="mx-auto font-bold text-gray-900 text-3xl font-[Poppins] select-none">Bubble</h2>
				</div>
				<div className="w-full px-[50px] sm:px-[30px] h-full overflow-y-auto">
					...
					<div className="mt-5 flex flex-col gap-6 pb-3">
						{/* Username Section */}
						<div>
							<div className={`flex flex-col gap-2 w-full py-3 px-2 border rounded-md ${isUsernameInvalid && "border-red-400 text-red-400"}`}>
								...
								<input type="text" name="username" maxLength={50} autoCapitalize="sentences" spellCheck={true} placeholder="Van Gogh" className={`font-[DM Sans] outline-none w-full ${isUsernameInvalid && "placeholder-red-300"}`} onChange={handleChangeUsername} value={username} required />
							</div>
							{ isUsernameInvalid && 
								<p className="text-red-400 text-xs select-none mt-1">Seu nome precisa ter no mínimo 8 caracteres.</p>
							}
							<p className="text-gray-500 text-xs select-none mt-1">Isso será exibido publicamente.</p>
						</div>
						{/* E-Mail Section */}
						<div>
							<div className={`flex flex-col gap-2 w-full py-3 px-2 border rounded-md ${isEmailInvalid && "border-red-400 text-red-400"}`}>
								<div className="flex justify-between items-center text-xs select-none">
									<label htmlFor="email" className="flex items-center gap-1"><MdAlternateEmail /> E-Mail</label>
								</div>
								<input type="email" name="email" autoCapitalize="sentences" spellCheck={true} placeholder="vg@gmail.com" className={`font-[DM Sans] outline-none w-full ${isEmailInvalid && "placeholder-red-300"}`} onChange={handleChangeEmail} value={email} required />
							</div>
							{ isEmailInvalid && 
								<p className="text-red-400 text-xs select-none mt-1">Tente providenciar um e-mail nesse formato: nome@host.com</p>
							}
							<p className="text-gray-500 text-xs select-none mt-1">Isso não será exibido publicamente. Você deve habilitar nas opções.</p>
						</div>
						{/* Password Section */}
						<div>
							<div className={`flex flex-col gap-2 w-full py-3 px-2 border rounded-md ${isPasswordInvalid && "border-red-400 text-red-400"}`}>
								<div className="flex justify-between items-center text-xs select-none">
								...
								<div className="flex items-center gap-2">
									<input type={`${isPasswordVisible == 0 ? "password" : "text"}`} name="password" minLength={5} autoComplete="current-password" placeholder="eusouovangogh123" className={`font-[DM Sans] outline-none w-full ${isPasswordInvalid && "placeholder-red-300"}`} onChange={handleChangePassword} value={password} required />
									<span
										onMouseDown={(e) => { 
											e.preventDefault(); 
							...
							{ isPasswordInvalid && 
								<p className="text-red-400 text-xs select-none mt-1">Sua senha deve conter caracteres maiúsculos e números.</p>
							}
						</div>
					</div>
				</div>
				<div className="w-full flex flex-col items-center justify-center px-7 py-4 border-t">
					...
					{ !formIsOk && 
						<p className="text-red-400 text-xs select-none mt-1">Você não completou o formulário corretamente.</p>
					}
				</div>
			</div>
		</div>
